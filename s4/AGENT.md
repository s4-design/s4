...
4. **Псевдокласс** (`:hover`) → готовое имя из словаря (`background-color--positive--mute:hover`), не конструировать.
...
6. **Ф2 — только если значение отличается от базового элемента** на устройстве. Одинаково везде → одна Ф1 (не дублировать в Ф2). Совпадает с базовым → класс не нужен.
