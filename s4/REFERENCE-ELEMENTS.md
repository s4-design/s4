...
2. **Атрибут `class`** — список через пробел: модификаторы стилизации (`second`, `danger`, `l1`…) и/или класс-дубликат `.element--*`. Порядок в списке не важен, значения при парсинге нормализуются.
...
**Правило композиции.** Если внутри контейнера есть совмещённые дети (класс-дубликат даёт им вес `0.1.1`), родитель **должен иметь вес выше**, чтобы покрывать детей и применять к ним изменения внешнего вида (углы, отступы). Для этого родителя усиливают его классом-дубликатом и утилитами:
...
**Класс-дубликат:** `.element--line` (на `<div>`).
...
**Класс-дубликат:** `.element--truncate` (на `<p>`/`<div>`).
...
| `title` | полный текст (для класс-дубликата) |
...
**Структура:** `<pre><code>…</code></pre>` (внутри — `<code>`).
